(function () {
  ...
  async function evaluateWithAdvisor(payload, opts = {}) {
    const params = new URLSearchParams({
      remaining_drawdown: opts.remaining_drawdown ?? 500,
      risk_cap_pct: opts.risk_cap_pct ?? 0.1,
      require_grade: opts.require_grade ?? 'A+',
      require_micro: opts.require_micro ?? false,
    });
    const res = await fetch(`/chat/advisor/evaluate?${params.toString()}`, {
      method: 'POST',
    ...
    if (!res.ok) {
      ...
      throw new Error(`Advisor request failed: ${res.status} ${text}`);
    }
    return res.json();
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const advBtn = document.getElementById('advisor_test');
    if (advBtn) {
      advBtn.addEventListener('click', async () => {
        try {
          ...
          const resp = await evaluateWithAdvisor(sample);
          alert(`Advisor: ${resp.decision} (grade ${resp.grade}, rule ${resp.rule})`);
        } catch (err) {
